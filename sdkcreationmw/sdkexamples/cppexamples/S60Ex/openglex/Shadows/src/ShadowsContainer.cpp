...
// ---------------------------------------------------------
// CShadowsContainer::ConstructL
// EPOC two phased constructor
// ---------------------------------------------------------
//
void CShadowsContainer::ConstructL(const TRect& aRect, CAknAppUi* aAppUi)
    {
    iAppUi = aAppUi;
    ...
    iShadows = CShadows::NewL( size.iWidth, size.iHeight );
    iShadows->AppInit();
    ...
    iPeriodic = CPeriodic::NewL( CActive::EPriorityIdle ); // Create an active object for
                                                           // animating the scene
    iPeriodic->Start( 100, 100, TCallBack( DrawCallBack, this ) );
}

// Destructor
CShadowsContainer::~CShadowsContainer()
    {
    delete iPeriodic;
...
// ---------------------------------------------------------
// CShadowsContainer::SizeChanged
// Called by framework when the view size is changed
// ---------------------------------------------------------
//
void CShadowsContainer::SizeChanged()
    {
    if( iOpenGlInitialized && iShadows )
        {
        TSize size;
        size = this->Size();

        iShadows->SetScreenSize( size.iWidth, size.iHeight );
        }
    }


// ---------------------------------------------------------
// CShadowsContainer::HandleResourceChange(
//     TInt aType)
// Dynamic screen resize changes by calling the
// SetExtentToWholeScreen() method again.
// ---------------------------------------------------------
//
 void CShadowsContainer::HandleResourceChange(TInt aType)
    {
	switch( aType )
    	{
...
// ---------------------------------------------------------
// CShadowsContainer::CountComponentControls
// ---------------------------------------------------------
//
TInt CShadowsContainer::CountComponentControls() const
    {
    return 0; // return nbr of controls inside this container
    }

// ---------------------------------------------------------
// CShadowsContainer::ComponentControl
// ---------------------------------------------------------
//
CCoeControl* CShadowsContainer::ComponentControl(TInt /*aIndex*/ ) const
    {
    return NULL;
    }

// ---------------------------------------------------------
// CShadowsContainer::Draw
// ---------------------------------------------------------
//
void CShadowsContainer::Draw(const TRect& /*aRect*/ ) const
    {
    // No need to implement anything here!
    }

// ---------------------------------------------------------
// CShadowsContainer::DrawCallBack
// Called by the CPeriodic in order to draw the graphics
// ---------------------------------------------------------
//
int CShadowsContainer::DrawCallBack( TAny* aInstance )
    {
    CShadowsContainer* instance = (CShadowsContainer*) aInstance;
    instance->iFrame++;

    // Call the main OpenGL ES Symbian rendering 'loop'
    instance->iShadows->AppCycle( instance->iFrame );
...
// ---------------------------------------------------------
// CShadowsContainer::HandleControlEventL
// ---------------------------------------------------------
//
void CShadowsContainer::HandleControlEventL(
