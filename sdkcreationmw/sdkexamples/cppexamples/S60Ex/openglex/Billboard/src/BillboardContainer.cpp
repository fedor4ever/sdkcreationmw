...
// ---------------------------------------------------------
// CBillboardContainer::ConstructL(const TRect& aRect)
// EPOC two phased constructor
// ---------------------------------------------------------
//
void CBillboardContainer::ConstructL(const TRect& aRect, CAknAppUi* aAppUi)
    {
    iAppUi = aAppUi;
    // Open GL hasn't been initialized
    iOpenGlInitialized = EFalse;
    // Create the input handler
    iInputHandler = CBillboardInput::NewL();
    ...
    iBillboard = CBillboard::NewL( size.iWidth, size.iHeight, iInputHandler );
    iBillboard->AppInitL();
    ...
    iPeriodic->Start( 100, 100, TCallBack( CBillboardContainer::DrawCallBack, this ) );
    }
...
// ------------------------------------------------------------------------------
//
TKeyResponse CBillboardContainer::OfferKeyEventL( const TKeyEvent& aKeyEvent,TEventCode aType )
    {
	switch( aType )
		{
		case EEventKeyDown:
...
// ---------------------------------------------------------
// CBillboardContainer::~CBillboardContainer()
...
// ---------------------------------------------------------
// CBillboardContainer::SizeChanged()
// Called by framework when the view size is changed
// ---------------------------------------------------------
//
void CBillboardContainer::SizeChanged()
    {
    if( iOpenGlInitialized && iBillboard )
        {
        // If example is running and OpenGL ES is initialized
        // notify it of the screen size change
        TSize size = this->Size();
        iBillboard->SetScreenSize( size.iWidth, size.iHeight );
        }
    }


// ---------------------------------------------------------
// CBillboardContainer::HandleResourceChange(
//     TInt aType)
// Dynamic screen resize changes by calling the
// SetExtentToWholeScreen() method again.
// ---------------------------------------------------------
//
 void CBillboardContainer::HandleResourceChange(TInt aType)
    {
    switch( aType )
        {
...
// ---------------------------------------------------------
// CBillboardContainer::CountComponentControls() const
// Return number of controls inside this container
// ---------------------------------------------------------
//
TInt CBillboardContainer::CountComponentControls() const
    {
    return 0;
    }

// ---------------------------------------------------------
// CBillboardContainer::ComponentControl(TInt aIndex) const
// ---------------------------------------------------------
//
CCoeControl* CBillboardContainer::ComponentControl(TInt /*aIndex*/) const
    {
    return NULL;
    }

// ---------------------------------------------------------
// CBillboardContainer::Draw(const TRect& aRect) const
// ---------------------------------------------------------
//
void CBillboardContainer::Draw(const TRect& aRect) const
    {
    CWindowGc& gc = SystemGc();
    gc.Clear( aRect );

    if ( iBillboard->GetState() == CBillboard::ELoadingTextures )
        {
...
// ---------------------------------------------------------
// CBillboardContainer::HandleControlEventL(
//     CCoeControl* aControl,TCoeEvent aEventType)
// ---------------------------------------------------------
//
void CBillboardContainer::HandleControlEventL(
    CCoeControl* /*aControl*/,TCoeEvent /*aEventType*/)
    {
...
// ----------------------------------------------------------------------
// CBillboardContainer::GetTimeTick
// Returns the current time.
// ----------------------------------------------------------------------
//
TTime CBillboardContainer::GetTimeTick()
    {
    TTime time;
    time.UniversalTime();
    return time;
...
// ---------------------------------------------------------
// CBillboardContainer::DrawCallBack( TAny* aInstance )
// Called by the CPeriodic in order to draw the graphics
// ---------------------------------------------------------
//
TInt CBillboardContainer::DrawCallBack( TAny* aInstance )
    {
    // Cast parameter to container instance
    CBillboardContainer* instance = (CBillboardContainer*) aInstance;
    ...
    if ( instance->iBillboard->GetState() == CBillboard::ERunning )
        {
        // Call the main OpenGL ES Symbian rendering 'loop'
        instance->iBillboard->AppCycle( instance->iFrame, timeSecs, deltaTimeSecs );
