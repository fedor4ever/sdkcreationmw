...
// ---------------------------------------------------------
//
void CSnowContainer::ConstructL(const TRect& aRect, CAknAppUi* aAppUi)
    {
    iAppUi = aAppUi;
    ...
    iSnow = CSnow::NewL( size.iWidth, size.iHeight );
    iSnow->AppInitL();
...
// ----------------------------------------------------------------------
// CSnowContainer::GetTimeTick
// Returns the current time.
// ----------------------------------------------------------------------
//
TTime CSnowContainer::GetTimeTick()
    {
    TTime time;
    time.UniversalTime();
    return time;
...
TInt CSnowContainer::DrawCallBack( TAny* aInstance )
    ...
    if ( instance->iSnow->GetState() == CSnow::ERunning )
        ...
        instance->iSnow->AppCycle( instance->iFrame, timeSecs, deltaTimeSecs );
