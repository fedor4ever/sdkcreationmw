...
// ---------------------------------------------------------
// CBumpMapContainer::ConstructL(const TRect& aRect)
// EPOC two phased constructor
// ---------------------------------------------------------
//
void CBumpMapContainer::ConstructL(const TRect& aRect, CAknAppUi* aAppUi)
    {
    iAppUi = aAppUi;
    ...
    iBumpMap = CBumpMap::NewL( size.iWidth, size.iHeight );
    iBumpMap->AppInitL();
    ...
    iPeriodic->Start( 100, 100, TCallBack( CBumpMapContainer::DrawCallBack, this ) );
    }

// ---------------------------------------------------------
// CBumpMapContainer::~CBumpMapContainer()
...
// ---------------------------------------------------------
// CBumpMapContainer::SizeChanged()
// Called by framework when the view size is changed
// ---------------------------------------------------------
//
void CBumpMapContainer::SizeChanged()
    {
    if( iOpenGlInitialized && iBumpMap )
        {
        // If example is running and OpenGL ES is initialized
        // notify it of the screen size change
        TSize size = this->Size();
        iBumpMap->SetScreenSize( size.iWidth, size.iHeight );
        }
    }


// ---------------------------------------------------------
// CBumpMapContainer::HandleResourceChange(
//     TInt aType)
// Dynamic screen resize changes by calling the
// SetExtentToWholeScreen() method again.
// ---------------------------------------------------------
//
 void CBumpMapContainer::HandleResourceChange(TInt aType)
    {
    switch( aType )
       {
...
// ---------------------------------------------------------
// CBumpMapContainer::CountComponentControls() const
// Return number of controls inside this container
// ---------------------------------------------------------
//
TInt CBumpMapContainer::CountComponentControls() const
    {
    return 0;
    }

// ---------------------------------------------------------
// CBumpMapContainer::ComponentControl(TInt aIndex) const
// ---------------------------------------------------------
//
CCoeControl* CBumpMapContainer::ComponentControl(TInt /*aIndex*/) const
    {
    return NULL;
    }

// ---------------------------------------------------------
// CBumpMapContainer::Draw(const TRect& aRect) const
// ---------------------------------------------------------
//
void CBumpMapContainer::Draw(const TRect& aRect) const
    {
    CWindowGc& gc = SystemGc();
    gc.Clear( aRect );

    if ( iBumpMap->GetState() == CBumpMap::ELoadingTextures )
        {
...
// ---------------------------------------------------------
// CBumpMapContainer::HandleControlEventL(
//     CCoeControl* aControl,TCoeEvent aEventType)
// ---------------------------------------------------------
//
void CBumpMapContainer::HandleControlEventL(
    CCoeControl* /*aControl*/,TCoeEvent /*aEventType*/)
    {
...
// ----------------------------------------------------------------------
// CBumpMapContainer::GetTimeTick
// Returns the current time.
// ----------------------------------------------------------------------
//
TTime CBumpMapContainer::GetTimeTick()
    {
    TTime time;
    time.UniversalTime();
    return time;
...
// ---------------------------------------------------------
// CBumpMapContainer::DrawCallBack( TAny* aInstance )
// Called by the CPeriodic in order to draw the graphics
// ---------------------------------------------------------
//
TInt CBumpMapContainer::DrawCallBack( TAny* aInstance )
    {
    // Cast parameter to container instance
    CBumpMapContainer* instance = (CBumpMapContainer*) aInstance;
    ...
    if ( instance->iBumpMap->GetState() == CBumpMap::ERunning )
        {
        // Call the main OpenGL ES Symbian rendering 'loop'
        instance->iBumpMap->AppCycle( instance->iFrame, timeSecs, deltaTimeSecs );
