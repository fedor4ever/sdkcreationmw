...
// Destructor
CSimpleLightContainer::~CSimpleLightContainer()
    {
    delete iPeriodic;

    if ( iSimpleLight )
        {
        iSimpleLight->AppExit();
        delete iSimpleLight;
        }
...
// ---------------------------------------------------------
// CSimpleLightContainer::SizeChanged()
// Called by framework when the view size is changed
// ---------------------------------------------------------
//
void CSimpleLightContainer::SizeChanged()
    {
    if( iOpenGlInitialized && iSimpleLight )
        {
        TSize size;
        size = this->Size();

        iSimpleLight->SetScreenSize( size.iWidth, size.iHeight );
        }
    }
...
// ---------------------------------------------------------
// CSimpleLightContainer::CountComponentControls() const
// ---------------------------------------------------------
//
TInt CSimpleLightContainer::CountComponentControls() const
    {
    return 0; // return nbr of controls inside this container
    }

// ---------------------------------------------------------
// CSimpleLightContainer::ComponentControl(TInt aIndex) const
// ---------------------------------------------------------
//
CCoeControl* CSimpleLightContainer::ComponentControl(TInt /*aIndex*/ ) const
    {
    return NULL;
    }

// ---------------------------------------------------------
// CSimpleLightContainer::Draw(const TRect& aRect) const
// ---------------------------------------------------------
//
void CSimpleLightContainer::Draw(const TRect& /*aRect*/ ) const
    {
    // No need to implement anything here!
    }

// ---------------------------------------------------------
// CSimpleLightContainer::DrawCallBack( TAny* aInstance )
// Called by the CPeriodic in order to draw the graphics
// ---------------------------------------------------------
//
int CSimpleLightContainer::DrawCallBack( TAny* aInstance )
    {
    CSimpleLightContainer* instance = (CSimpleLightContainer*) aInstance;
    instance->iFrame++;

    // Call the main OpenGL ES Symbian rendering 'loop'
    instance->iSimpleLight->AppCycle( instance->iFrame );
...
// ---------------------------------------------------------
// CSimpleLightContainer::HandleControlEventL(
//     CCoeControl* aControl,TCoeEvent aEventType)
// ---------------------------------------------------------
//
void CSimpleLightContainer::HandleControlEventL(
    CCoeControl* /*aControl*/,TCoeEvent /*aEventType*/)
    {
    }
