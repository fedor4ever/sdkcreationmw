...
// ---------------------------------------------------------
// CEnvMappingContainer::ConstructL(const TRect& aRect)
// Symbian two phased constructor
// ---------------------------------------------------------
//
void CEnvMappingContainer::ConstructL(const TRect& aRect, CAknAppUi* aAppUi)
    {
    iAppUi = aAppUi;
    ...
    iEnvMapping = CEnvMapping::NewL( size.iWidth, size.iHeight );
    iEnvMapping->AppInitL();

	iOpenGlInitialized = ETrue;

    iPeriodic = CPeriodic::NewL( CActive::EPriorityIdle );     // Create an active object for
                                                               // animating the scene
    iPeriodic->Start( 100, 100, TCallBack( CEnvMappingContainer::DrawCallBack, this ) );
}

// Destructor
CEnvMappingContainer::~CEnvMappingContainer()
{
    delete iPeriodic;
...
// ---------------------------------------------------------
// CEnvMappingContainer::SizeChanged()
// Called by framework when the view size is changed
// ---------------------------------------------------------
//
void CEnvMappingContainer::SizeChanged()
    {
    if( iOpenGlInitialized && iEnvMapping )
        {
        TSize size;
        size = this->Size();

        iEnvMapping->SetScreenSize( size.iWidth, size.iHeight );
        }
    }

// ---------------------------------------------------------
// CEnvMappingContainer::CountComponentControls() const
// ---------------------------------------------------------
//
TInt CEnvMappingContainer::CountComponentControls() const
{
    return 0;
}

// ---------------------------------------------------------
// CEnvMappingContainer::ComponentControl(TInt aIndex) const
// ---------------------------------------------------------
//
CCoeControl* CEnvMappingContainer::ComponentControl(TInt /*aIndex*/ ) const
{
    return NULL;
}

// ---------------------------------------------------------
// CEnvMappingContainer::Draw(const TRect& aRect) const
// ---------------------------------------------------------
//
void CEnvMappingContainer::Draw(const TRect& /*aRect*/ ) const
{
    // No need to implement anything here!
}

// ---------------------------------------------------------
// CEnvMappingContainer::DrawCallBack( TAny* aInstance )
// Called by the CPeriodic in order to draw the graphics
// ---------------------------------------------------------
//
int CEnvMappingContainer::DrawCallBack( TAny* aInstance )
{
    CEnvMappingContainer* instance = (CEnvMappingContainer*) aInstance;

    if ( instance->iEnvMapping->GetState() == CEnvMapping::ERunning )
    {
        instance->iFrame++;

        /* Call the main OpenGL ES Symbian rendering 'loop' */
        instance->iEnvMapping->AppCycle( instance->iFrame );
...
// ---------------------------------------------------------
// CEnvMappingContainer::HandleControlEventL(
//     CCoeControl* aControl,TCoeEvent aEventType)
// ---------------------------------------------------------
//
void CEnvMappingContainer::HandleControlEventL(
    CCoeControl* /*aControl*/,TCoeEvent /*aEventType*/)
{
}

// ---------------------------------------------------------
// CEnvMappingContainer::HandleResourceChange(
//     TInt aType)
// Dynamic screen resize changes by calling the
// SetExtentToWholeScreen() method again.
// ---------------------------------------------------------
//
 void CEnvMappingContainer::HandleResourceChange(TInt aType)
    {
	switch( aType )
    	{
