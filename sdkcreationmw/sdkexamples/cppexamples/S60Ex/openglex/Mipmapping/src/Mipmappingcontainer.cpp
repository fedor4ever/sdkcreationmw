...
// ---------------------------------------------------------
// CMipmappingContainer::ConstructL(const TRect& aRect)
// EPOC two phased constructor
// ---------------------------------------------------------
//
void CMipmappingContainer::ConstructL(const TRect& aRect, CAknAppUi* aAppUi)
    {
    iAppUi = aAppUi;
...
// Do not go to full-screen in touch devices.    
    ...
    TSize size;
    size = this->Size();
    iMipmapping = CMipmapping::NewL( size.iWidth, size.iHeight );
    iMipmapping->AppInitL();

    iOpenGlInitialized = ETrue;

	// Create an active object for animating the scene
    iPeriodic = CPeriodic::NewL( CActive::EPriorityIdle );
    iPeriodic->Start( 100, 100, TCallBack( CMipmappingContainer::DrawCallBack, this ) );
    }

// ---------------------------------------------------------
// CMipmappingContainer::~CMipmappingContainer()
...
// ---------------------------------------------------------
// CMipmappingContainer::SizeChanged()
// Called by framework when the view size is changed
// ---------------------------------------------------------
//
void CMipmappingContainer::SizeChanged()
    {
    if( iOpenGlInitialized && iMipmapping )
        {
        TSize size;
        size = this->Size();

        iMipmapping->SetScreenSize( size.iWidth, size.iHeight );
        }
    }


// ---------------------------------------------------------
// CMipmappingContainer::HandleResourceChange(
//     TInt aType)
// Dynamic screen resize changes by calling the
// SetExtentToWholeScreen() method again.
// ---------------------------------------------------------
//
 void CMipmappingContainer::HandleResourceChange(TInt aType)
    {
	switch( aType )
    	{
...
// ---------------------------------------------------------
// CMipmappingContainer::CountComponentControls() const
// Return number of controls inside this container
// ---------------------------------------------------------
//
TInt CMipmappingContainer::CountComponentControls() const
    {
    return 0;
    }

// ---------------------------------------------------------
// CMipmappingContainer::ComponentControl(TInt aIndex) const
// ---------------------------------------------------------
//
CCoeControl* CMipmappingContainer::ComponentControl(TInt /*aIndex*/) const
    {
    return NULL;
    }

// ---------------------------------------------------------
// CMipmappingContainer::Draw(const TRect& aRect) const
// ---------------------------------------------------------
//
void CMipmappingContainer::Draw(const TRect& aRect) const
	{
    CWindowGc& gc = SystemGc();
    gc.Clear( aRect );

    if ( iMipmapping->GetState() == CMipmapping::ELoadingTextures )
		{
...
// ---------------------------------------------------------
// CMipmappingContainer::HandleControlEventL(
//     CCoeControl* aControl,TCoeEvent aEventType)
// ---------------------------------------------------------
//
void CMipmappingContainer::HandleControlEventL(
    CCoeControl* /*aControl*/,TCoeEvent /*aEventType*/)
    {
...
// ----------------------------------------------------------------------
// CMipmappingContainer::GetTimeTick
// Returns the current time.
// ----------------------------------------------------------------------
//
TTime CMipmappingContainer::GetTimeTick()
    {
    TTime time;
    time.UniversalTime();
    return time;
...
// ---------------------------------------------------------
// CMipmappingContainer::DrawCallBack( TAny* aInstance )
// Called by the CPeriodic in order to draw the graphics
// ---------------------------------------------------------
//
TInt CMipmappingContainer::DrawCallBack( TAny* aInstance )
	{
    CMipmappingContainer* instance = (CMipmappingContainer*) aInstance;
    ...
    if ( instance->iMipmapping->GetState() == CMipmapping::ERunning )
		{
        // Call the main OpenGL ES Symbian rendering 'loop'
        instance->iMipmapping->AppCycle( instance->iFrame, timeSecs, deltaTimeSecs );
