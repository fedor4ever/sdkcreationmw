# Microsoft Developer Studio Project File - Name="xrpcrt" - Package Owner=<4>
# Microsoft Developer Studio Generated Build File, Format Version 6.00
# ** DO NOT EDIT **

# TARGTYPE "Win32 (x86) Dynamic-Link Library" 0x0102

CFG=xrpcrt - Win32 Debug
!MESSAGE This is not a valid makefile. To build this project using NMAKE,
!MESSAGE use the Export Makefile command and run
!MESSAGE 
!MESSAGE NMAKE /f "xrpcrt.mak".
!MESSAGE 
!MESSAGE You can specify a configuration when running NMAKE
!MESSAGE by defining the macro CFG on the command line. For example:
!MESSAGE 
!MESSAGE NMAKE /f "xrpcrt.mak" CFG="xrpcrt - Win32 Debug"
!MESSAGE 
!MESSAGE Possible choices for configuration are:
!MESSAGE 
!MESSAGE "xrpcrt - Win32 Release" (based on "Win32 (x86) Dynamic-Link Library")
!MESSAGE "xrpcrt - Win32 Debug" (based on "Win32 (x86) Dynamic-Link Library")
!MESSAGE 

# Begin Project
# PROP AllowPerConfigDependencies 0
# PROP Scc_ProjName ""
# PROP Scc_LocalPath ""
CPP=cl.exe
MTL=midl.exe
RSC=rc.exe

!IF  "$(CFG)" == "xrpcrt - Win32 Release"

# PROP BASE Use_MFC 0
# PROP BASE Use_Debug_Libraries 0
# PROP BASE Output_Dir "Release"
# PROP BASE Intermediate_Dir "Release"
# PROP BASE Target_Dir ""
# PROP Use_MFC 0
# PROP Use_Debug_Libraries 0
# PROP Output_Dir "build\release"
# PROP Intermediate_Dir "build\release"
# PROP Ignore_Export_Lib 0
# PROP Target_Dir ""
# ADD BASE CPP /nologo /MT /W3 /GX /O2 /D "WIN32" /D "NDEBUG" /D "_WINDOWS" /D "_MBCS" /D "_USRDLL" /D "XRPCRT_EXPORTS" /YX /FD /c
# ADD CPP /nologo /MD /W3 /GX /O2 /I "../../../NmitShared/src/native" /I "../include" /D "WIN32" /D "NDEBUG" /D "_WINDOWS" /D "_MBCS" /D "_USRDLL" /D "XRPCRT_EXPORTS" /YX /FD /c
# ADD BASE MTL /nologo /D "NDEBUG" /mktyplib203 /win32
# ADD MTL /nologo /D "NDEBUG" /mktyplib203 /win32
# ADD BASE RSC /l 0x409 /d "NDEBUG"
# ADD RSC /l 0x409 /d "NDEBUG"
BSC32=bscmake.exe
# ADD BASE BSC32 /nologo
# ADD BSC32 /nologo
LINK32=link.exe
# ADD BASE LINK32 kernel32.lib user32.lib gdi32.lib winspool.lib comdlg32.lib advapi32.lib shell32.lib ole32.lib oleaut32.lib uuid.lib odbc32.lib odbccp32.lib /nologo /dll /machine:I386
# ADD LINK32 kernel32.lib user32.lib advapi32.lib ws2_32.lib /nologo /base:"0x21000000" /dll /machine:I386
# Begin Special Build Tool
OutDir=.\build\release
TargetPath=.\build\release\xrpcrt.dll
TargetName=xrpcrt
SOURCE="$(InputPath)"
PostBuild_Desc=Copying release files...
PostBuild_Cmds=if exist ..\..\..\..\epoc32\release\wins\urel echo ..\..\..\..\epoc32\release\wins\urel\$(TargetName).dll	if exist ..\..\..\..\epoc32\release\wins\urel echo ..\..\..\..\epoc32\release\wins\urel\$(TargetName).lib	if exist ..\..\..\..\epoc32\release\wins\urel copy $(TargetPath) ..\..\..\..\epoc32\release\wins\urel\$(TargetName).dll	if exist ..\..\..\..\epoc32\release\wins\urel copy $(OutDir)\$(TargetName).lib ..\..\..\..\epoc32\release\wins\urel\$(TargetName).lib	if exist ..\..\..\..\epoc32\release\winscw\urel echo ..\..\..\..\epoc32\release\winscw\urel\$(TargetName).dll	if exist ..\..\..\..\epoc32\release\winscw\urel echo ..\..\..\..\epoc32\release\winscw\urel\$(TargetName).lib	if exist ..\..\..\..\epoc32\release\winscw\urel copy $(TargetPath) ..\..\..\..\epoc32\release\winscw\urel\$(TargetName).dll	if exist ..\..\..\..\epoc32\release\winscw\urel copy $(OutDir)\$(TargetName).lib ..\..\..\..\epoc32\release\winscw\urel\$(TargetName).lib	if exist ..\..\..\..\epoc32\release\winscw\udeb echo ..\..\..\..\epoc32\release\winscw\udeb\$(TargetName).dll	if exist ..\..\..\..\epoc32\release\winscw\udeb echo ..\..\..\..\epoc32\release\winscw\udeb\$(TargetName).lib	if exist\
 ..\..\..\..\epoc32\release\winscw\udeb copy $(TargetPath)    ..\..\..\..\epoc32\release\winscw\udeb\$(TargetName).dll	if exist ..\..\..\..\epoc32\release\winscw\udeb copy $(OutDir)\$(TargetName).lib ..\..\..\..\epoc32\release\winscw\udeb\$(TargetName).lib
# End Special Build Tool

!ELSEIF  "$(CFG)" == "xrpcrt - Win32 Debug"

# PROP BASE Use_MFC 0
# PROP BASE Use_Debug_Libraries 1
# PROP BASE Output_Dir "Debug"
# PROP BASE Intermediate_Dir "Debug"
# PROP BASE Target_Dir ""
# PROP Use_MFC 0
# PROP Use_Debug_Libraries 1
# PROP Output_Dir "build\debug"
# PROP Intermediate_Dir "build\debug"
# PROP Ignore_Export_Lib 0
# PROP Target_Dir ""
# ADD BASE CPP /nologo /MTd /W3 /Gm /GX /ZI /Od /D "WIN32" /D "_DEBUG" /D "_WINDOWS" /D "_MBCS" /D "_USRDLL" /D "XRPCRT_EXPORTS" /YX /FD /GZ /c
# ADD CPP /nologo /MDd /W3 /Gm /GX /Zi /Od /I "../../NmitShared/src/native" /I "../include" /D "WIN32" /D "_DEBUG" /D "_WINDOWS" /D "_MBCS" /D "_USRDLL" /D "XRPCRT_EXPORTS" /FR /YX /FD /GZ /c
# ADD BASE MTL /nologo /D "_DEBUG" /mktyplib203 /win32
# ADD MTL /nologo /D "_DEBUG" /mktyplib203 /win32
# ADD BASE RSC /l 0x409 /d "_DEBUG"
# ADD RSC /l 0x409 /d "_DEBUG"
BSC32=bscmake.exe
# ADD BASE BSC32 /nologo
# ADD BSC32 /nologo
LINK32=link.exe
# ADD BASE LINK32 kernel32.lib user32.lib gdi32.lib winspool.lib comdlg32.lib advapi32.lib shell32.lib ole32.lib oleaut32.lib uuid.lib odbc32.lib odbccp32.lib /nologo /dll /debug /machine:I386 /pdbtype:sept
# ADD LINK32 kernel32.lib user32.lib advapi32.lib ws2_32.lib /nologo /base:"0x21000000" /dll /debug /machine:I386 /pdbtype:sept
# Begin Special Build Tool
OutDir=.\build\debug
TargetPath=.\build\debug\xrpcrt.dll
TargetName=xrpcrt
SOURCE="$(InputPath)"
PostBuild_Desc=Copying debug files...
PostBuild_Cmds=if exist ..\..\..\..\epoc32\release\wins\udeb echo ..\..\..\..\epoc32\release\wins\udeb\$(TargetName).dll	if exist ..\..\..\..\epoc32\release\wins\udeb echo ..\..\..\..\epoc32\release\wins\udeb\$(TargetName).lib	if exist ..\..\..\..\epoc32\release\wins\udeb copy $(TargetPath) ..\..\..\..\epoc32\release\wins\udeb\$(TargetName).dll	if exist ..\..\..\..\epoc32\release\wins\udeb copy $(OutDir)\$(TargetName).lib ..\..\..\..\epoc32\release\wins\udeb\$(TargetName).lib
# End Special Build Tool

!ENDIF 

# Begin Target

# Name "xrpcrt - Win32 Release"
# Name "xrpcrt - Win32 Debug"
# Begin Group "Source Files"

# PROP Default_Filter "cpp;c;cxx;rc;def;r;odl;idl;hpj;bat"
# Begin Source File

SOURCE=..\src\xrpcrt.c
# End Source File
# Begin Source File

SOURCE=..\src\xrpcrt.def
# End Source File
# End Group
# Begin Group "Header Files"

# PROP Default_Filter "h;hpp;hxx;hm;inl"
# Begin Source File

SOURCE=..\include\xrpcrt.h
# End Source File
# End Group
# End Target
# End Project
