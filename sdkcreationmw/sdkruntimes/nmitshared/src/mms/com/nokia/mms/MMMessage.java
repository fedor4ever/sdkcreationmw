...
public class MMMessage implements IMMConstants, Serializable {
  ...
  private boolean flagContentLocAvailable = false;
  ...
  private boolean flagMessageSizeAvailable = false;
  ...
  private String hContentLoc = "";
  private byte hStatus;
  private int hSize;

  // User Defined MMHeaders
  private Hashtable hUserDefinedList = null;
  ...
  // associative list
  private Hashtable hTableOfContents = null;

  // ordered array
  private Vector hContentList = null;

  /**
   * Constructor
   */
  public MMMessage() {
    hTableOfContents = new Hashtable();
    hContentList = new Vector(3);
    hUserDefinedList = new Hashtable();
    hTo = new Vector();
    hCc = new Vector();
    hBcc = new Vector();
  }
  ...
  /**
   * Check if the X-Mms-Content-Location field is available (m-notification-ind only)
   *
   * return true if availale.
   */
  public boolean isContentLocAvailable() {
    return flagContentLocAvailable;
  }
  ...
  /**
   * Check if the Message Size header is specified.
   *
   * return true if Mms-Message-Size header is specified.
   */
  public boolean isMessageSizeAvailable() {
    return flagMessageSizeAvailable;
  }

  /**
   * Check if there is at least one user-defined header.
   *
   * return true if at least one header
   */
  public boolean isUserDefinedAvailable() {
    return (hUserDefinedList.size() > 0 ? true : false);
  }
  /**
  ...
  /**
   * Retrieves the content location header of the message (m-notification only)
   *
  ...
  /**
   * Sets the content location header of the message (m-notification only)
   *
   * param value is the URL for the content location. 
   *
   */
  public void setContentLocation(String value) {
    hContentLoc = new String( value );
    flagContentLocAvailable = true;
  }
  ...
  public void addContent(MMContent mmc) {
    hTableOfContents.put(mmc.getContentId(), mmc);
    hContentList.add(mmc);
  }
  ...
  public MMContent getPresentation() {
    if ((flagMultipartRelated==true) && (getNumContents()>0))
       return (MMContent) hTableOfContents.get(presentationId);
    else
       return null;
  }
  ...
  public MMContent getContent(String id) {
    return (MMContent) hTableOfContents.get(id);
  }

  /**
   * Retrieves the content at the position index.
   *
   * return the content or null if out of range.
   * param index is the index of the content to be retrieved.
   */
  public MMContent getContent(int index) {
    MMContent value = null;
    if (index < 0 || index >= hContentList.size())
        return null;
    value = (MMContent) hContentList.elementAt(index);
    return value;
  }
  ...
  /**
   * Retrieves the message size for the recipient (m-notification-ind only).
   *
  ...
  /**
   * Sets the message size for the recipient (m-notification-ind only).
   *
   * @param value Message size in bytes.
   *
   */
  public void setMessageSize(int value) {
    hSize = value;
    flagMessageSizeAvailable = true;
  }

  /**
   * Add a user-defined header.
   */
  public void addUserDefinedHeader(String name, String val) {
    if (name == null) {
        return;
    }
    name = name.trim();
    ...
    if (val == null) val = " ";
    hUserDefinedList.put(name, val);
  }

  /**
   * Get array of the names of the user-defined headers.
   */
  public String[] getUserDefinedHeaderNames() {
    String[] arr = new String[ hUserDefinedList.size() ];
    Enumeration en = hUserDefinedList.keys();
    int i = 0;
   ...
   * Get array of the names of the user-defined headers.
   */
  public String getUserDefinedHeaderValue(String name) {
    return (String)hUserDefinedList.get(name);
  }
}
